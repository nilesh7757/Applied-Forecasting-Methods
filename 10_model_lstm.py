...
def run_lstm_forecast():
    ...
    # Sequence creation (Last 10 seconds to predict next 1)
    def create_sequences(data, seq_length):
        X, y = [], []
        for i in range(len(data) - seq_length):
            X.append(data[i:i+seq_length])
            y.append(data[i+seq_length, 0]) # Index 0 is stress_score
    ...
    # 3. Model Setup
    model = StressLSTM(input_size=len(features), hidden_size=64, num_layers=2, output_size=1)
    ...
    # Inverse scaling for metrics (only for stress_score)
    # Reconstruct a dummy matrix for inverse transform
    dummy = np.zeros((len(predictions_scaled), len(features)))
    dummy[:, 0] = predictions_scaled.flatten()
    predictions = scaler.inverse_transform(dummy)[:, 0]
    
    # Inverse scaling for actual y
    ...
    plt.plot(predictions, color='#9467bd', linestyle='--', label='LSTM Forecast')
    plt.title('Deep Learning: LSTM Neural Network Stress Forecasting', fontweight='bold')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('Forecasting_Graphs/10_LSTM_Forecast.png', dpi=300)
    plt.close()
